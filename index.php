...
<head>
    ...
    <style>
        ...
        #navbar{z-index:9990;will-change:background,box-shadow,backdrop-filter}
        #navbar.gemu-navbar-open{z-index:10010;background:rgba(10,25,41,.96)!important;backdrop-filter:blur(18px);box-shadow:0 18px 40px rgba(0,0,0,.22)}
        #mobile-menu{display:block!important;position:fixed!important;top:0!important;right:0!important;bottom:0!important;width:min(390px,92vw)!important;max-width:min(92vw,390px)!important;overflow-y:auto;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;background:#0a1929!important;box-shadow:-24px 0 60px rgba(0,0,0,.32);z-index:10000!important;visibility:hidden;opacity:0;pointer-events:none;transform:translateX(110%)!important;transition:transform .32s cubic-bezier(.22,1,.36,1),opacity .22s ease,visibility .22s ease;will-change:transform,opacity}
        #mobile-menu.menu-open,#mobile-menu.gemu-menu-open{visibility:visible!important;opacity:1!important;pointer-events:auto!important;transform:translateX(0)!important}
        #mobile-menu-overlay{position:fixed!important;inset:0!important;z-index:9998!important;background:rgba(0,0,0,.56);opacity:0;pointer-events:none;transition:opacity .25s ease;will-change:opacity}
        #mobile-menu-overlay.gemu-overlay-open{display:block!important;opacity:1!important;pointer-events:auto!important}
        #mobile-menu-btn{position:relative;z-index:10020!important}
        #mobile-menu .mobile-link{text-decoration:none}
        @supports not (display:flex){#mobile-menu .mobile-link{display:block;color:rgba(255,255,255,.82);padding:.875rem 1rem;border-radius:.75rem}}
        body.menu-locked,html.menu-locked{overflow:hidden}
        @media(max-width:767px){
            #hero .max-w-3xl{max-width:100%}
            #hero h1{word-break:normal;overflow-wrap:anywhere}
            #about{scroll-margin-top:72px}
            #about .grid{gap:2rem}
            #about img{max-height:520px}
            #project-card{max-height:94dvh;border-radius:1rem}
            #project-card .overflow-y-auto{padding-bottom:1rem}
            #project-modal-frame{height:46vh!important;min-height:280px}
            .toast{left:.75rem;right:.75rem;width:auto}
        }
        @media(min-width:768px){
            #mobile-menu{width:min(390px,92vw)}
        }
...
<body class="bg-white text-navy-900 antialiased">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 pt-28 pb-20 sm:pt-32 sm:pb-24 md:pt-40 md:pb-32 relative z-10 w-full">
            <div class="max-w-3xl">
                ...
                <div class="flex items-center gap-4 mb-8 sm:mb-10">
                    <a href="https://www.linkedin.com/in/darmarakhaa" target="_blank" rel="noopener" class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/60 hover:text-[#0077b5] hover:border-[#0077b5]/50 hover:bg-[#0077b5]/10 transition-all duration-300 shadow-lg" title="LinkedIn">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-linkedin"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
                    </a>
                    <a href="https://github.com/darmarakha" target="_blank" rel="noopener" class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/60 hover:text-white hover:border-white/50 hover:bg-white/10 transition-all duration-300 shadow-lg" title="GitHub">
    ...
    <div id="project-modal" class="lightbox fixed inset-0 z-[105] bg-navy-950/95 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[92vh] flex flex-col overflow-hidden relative transition-transform transform scale-95 opacity-0" id="project-card">
            <button onclick="closeProjectModal()" class="absolute top-3 right-3 sm:top-4 sm:right-4 w-9 h-9 flex items-center justify-center rounded-full bg-white/90 hover:bg-navy-100 text-navy-600 transition-colors z-20 shadow-sm border border-navy-100" aria-label="Close">
            ...
            <div class="overflow-y-auto p-4 sm:p-6 md:p-8">
                <div class="grid lg:grid-cols-5 gap-5 sm:gap-7 items-start">
                    <div class="lg:col-span-2">
                        <p class="text-accent-600 font-semibold text-xs tracking-widest uppercase mb-2" data-i18n="proj.detailBtn">Detail Proyek</p>
                        <h3 class="text-xl sm:text-2xl md:text-3xl font-bold text-navy-900 mb-3 leading-tight" id="project-modal-title">Judul Proyek</h3>
                        <div class="flex flex-wrap gap-2 mb-4" id="project-modal-tags"></div>
                        <img id="project-modal-img" src="" alt="Screenshot Proyek" class="w-full h-auto max-h-72 object-contain bg-navy-50 rounded-xl mb-5 shadow-sm border border-navy-100">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="text-sm sm:text-base text-navy-600 leading-relaxed" id="project-modal-detail">
                    ...
                    <div class="lg:col-span-3">
                        <div class="rounded-2xl border border-navy-100 bg-navy-50/70 overflow-hidden" id="project-preview-container">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-b border-navy-100 bg-white">
                                <div class="min-w-0">
                                    <p class="text-xs font-semibold text-accent-600 uppercase tracking-widest" data-i18n="proj.filePreview">Preview File Proyek</p>
                                    <p class="text-sm font-bold text-navy-900 truncate" id="project-file-label">Project file</p>
                                </div>
                                <a href="#" target="_blank" rel="noopener" id="project-modal-link" class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-navy-900 text-white font-semibold rounded-xl hover:bg-navy-800 transition-colors text-xs sm:text-sm whitespace-nowrap">
                            ...
                            <div class="bg-white" id="project-preview-body">
                                <iframe
                                    id="project-modal-frame"
                                    src=""
                                    class="hidden w-full h-[55vh] bg-white"
                                    title="Project Preview"
                                    loading="lazy"
                                    sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-downloads"
                                ></iframe>
                                <img id="project-modal-preview-img" src="" alt="Preview File Proyek" class="hidden w-full h-auto max-h-[55vh] object-contain bg-white">
                                <div id="project-no-file" class="hidden p-6 text-sm text-navy-500 text-center">
                                    File proyek belum tersedia. Tambahkan <code class="px-1.5 py-0.5 bg-navy-100 rounded text-navy-700">fileUrl</code> di <code class="px-1.5 py-0.5 bg-navy-100 rounded text-navy-700">data.js</code>.
                                </div>
                            </div>
                        </div>
                    </div>
