...
<body class="h-screen flex flex-col overflow-hidden">
    ...
    <div class="flex-1 flex overflow-hidden">
        ...
        <aside class="w-72 bg-[#0d1117] border-r border-[#30363d] flex flex-col flex-shrink-0">
            ...
            <div class="p-5 border-t border-[#30363d]">
                <h2 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <i data-lucide="folder-open" class="w-3.5 h-3.5"></i> Upload File Proyek
                </h2>
                <div class="bg-[#161b22] border border-[#30363d] rounded-md p-4 text-center border-dashed">
                    <input type="file" id="project-upload" accept=".jpg,.jpeg,.png,.webp,.gif,.svg,.pdf,.html,.htm,.zip,.rar,.7z" class="hidden" onchange="uploadProjectFile()">
                    <label for="project-upload" class="cursor-pointer flex flex-col items-center gap-2 text-gray-400 hover:text-white transition-colors">
                        <i data-lucide="upload-cloud" class="w-6 h-6"></i>
                        <span class="text-xs font-medium" id="project-upload-text">Klik untuk pilih file proyek</span>
                    </label>
                </div>
                <p class="text-[10px] text-gray-500 mt-2 leading-relaxed">Bisa untuk gambar, PDF, HTML, ZIP, atau RAR. Copy URL ke <span class="text-blue-400">fileUrl</span> di data.js.</p>

                <div id="project-result" class="mt-4 hidden">
                    <p class="text-[10px] text-gray-400 mb-1">URL File Proyek (Copy paste ke data.js):</p>
                    <input type="text" id="project-url" readonly class="w-full bg-[#0d1117] border border-[#30363d] text-xs rounded-md px-2 py-1.5 text-blue-400 cursor-copy" onclick="this.select(); document.execCommand('copy'); alert('Link disalin!');">
                </div>
            </div>
        </aside>
    ...
    <script>
        ...
        async function uploadProjectFile() {
            ...
            const text = document.getElementById('project-upload-text');
            text.textContent = 'Mengupload file proyek...';
            ...
            try {
                ...
                alert('Gagal upload file proyek.');
            } finally {
                text.textContent = 'Klik untuk pilih file proyek';
                fileInput.value = '';
            }
        }
    </script>
