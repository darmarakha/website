...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&display=swap"
        rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@300;400;500;600;700;900&display=swap"
        rel="stylesheet">
    <script>
        tailwind.config = {
            theme: {
...
<body class="text-white min-h-screen selection:bg-cyan-500 selection:text-white pb-20"
    ...
    <div
        class="fixed top-[-10%] right-[-5%] w-[400px] h-[400px] bg-cyan-400/[.03] rounded-full blur-[150px] pointer-events-none">
    </div>
    <div
        class="fixed bottom-[-10%] left-[-5%] w-[350px] h-[350px] bg-emerald-500/[.03] rounded-full blur-[120px] pointer-events-none">
    </div>
    ...
    <main class="max-w-6xl mx-auto px-4 sm:px-6 py-6 md:py-10 space-y-6 relative z-10">
        ...
        <!-- HEADER -->
        <header
            class="glass-card rounded-3xl p-5 md:p-7 flex flex-col md:flex-row md:items-center gap-5 md:justify-between relative overflow-hidden">
            <div
                class="absolute -top-16 -right-16 w-48 h-48 bg-gradient-to-br from-cyan-400/10 to-transparent rounded-full blur-3xl pointer-events-none">
            </div>
            <div class="flex items-center gap-4">
                <div
                    class="w-14 h-14 rounded-2xl bg-gradient-to-br from-cyan-400 to-cyan-600 flex items-center justify-center font-jp font-black text-2xl text-white shadow-lg shadow-cyan-500/20 shrink-0">
                    ア</div>
                <div>
                    <h1 class="text-2xl md:text-3xl font-black tracking-tight">
                        <span class="bg-gradient-to-r from-cyan-300 to-emerald-400 bg-clip-text text-transparent">Materi
                            Katakana</span>
                    </h1>
            ...
            <div class="flex flex-col md:items-end gap-3 w-full md:w-auto">
                ...
                <?php else: ?>
                    <div
                        class="flex items-center justify-end gap-2 bg-amber-500/[.08] px-4 py-2.5 rounded-2xl border border-amber-500/15 text-amber-300 text-sm font-medium self-end w-full md:w-auto">
                        <i data-lucide="alert-circle" class="w-4 h-4"></i> Belum Login (Guest)
                    </div>
                <?php endif; ?>
                <div class="flex gap-2.5 w-full md:w-auto">
                    <a href="index.php"
                        class="flex-1 md:flex-none justify-center px-5 py-2.5 rounded-xl bg-white/[.05] hover:bg-white/[.1] text-neutral-300 font-semibold text-sm border border-white/[.05] hover:border-white/[.15] transition-all flex items-center gap-2">
                        <i data-lucide="arrow-left" class="w-4 h-4"></i> Kembali
                    </a>
                    <a href="/index.php"
                        class="flex-1 md:flex-none justify-center px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-emerald-500 hover:from-cyan-400 hover:to-emerald-400 text-white font-semibold text-sm shadow-lg shadow-cyan-500/15 flex items-center gap-2">
        ...
        <nav class="flex flex-wrap sm:flex-nowrap gap-2 glass-card p-2 rounded-2xl">
            <button onclick="switchTab('materi')" id="tab-materi"
                class="flex-1 min-w-[120px] py-3 px-3 rounded-xl font-bold text-sm transition-all bg-gradient-to-r from-cyan-500 to-cyan-600 text-white shadow-lg shadow-cyan-500/20">📚
                Materi</button>
            <button onclick="switchTab('flashcard')" id="tab-flashcard"
                class="flex-1 min-w-[120px] py-3 px-3 rounded-xl font-bold text-sm transition-all text-neutral-400 hover:text-white hover:bg-white/[.05]">🎴
                Flashcard</button>
            <button onclick="switchTab('ai')" id="tab-ai"
                class="flex-1 min-w-[120px] py-3 px-3 rounded-xl font-bold text-sm transition-all text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20">📖
                AI Membaca</button>
        </nav>
        ...
        <footer class="mt-12 text-center">
            <div class="flex items-center justify-center gap-2 mb-3">
                <div class="w-6 h-px bg-white/10"></div><span class="text-lg text-cyan-400">⚡</span>
                <div class="w-6 h-px bg-white/10"></div>
            </div>
            <p class="text-neutral-500 text-sm">Credit: <strong class="text-neutral-300">Darma</strong> — Dikembangkan
                dengan ❤️ & AI</p>
        </footer>

        <!-- ===== STROKE ORDER MODAL CANGGIH ===== -->
        <div id="strokeModal"
            class="hidden fixed inset-0 bg-dark-900/90 z-50 flex justify-center items-center px-4 backdrop-blur-sm transition-opacity">
            <div
                class="glass-card rounded-3xl p-6 sm:p-8 max-w-sm w-full relative shadow-2xl border border-cyan-400/20 flex flex-col items-center bg-dark-800">
                <button onclick="hideStrokeModal()"
                    class="absolute top-4 right-4 text-neutral-500 hover:text-white bg-white/[.05] hover:bg-white/[.1] p-2 rounded-xl w-9 h-9 flex items-center justify-center transition border border-white/[.05] text-sm z-10">✕</button>

                <h3 class="text-lg font-bold text-cyan-400 mb-4 text-center w-full border-b border-white/[.06] pb-3">
                    Papan Penulisan</h3>

                <div class="flex items-center gap-2 mb-5 w-full">
                    <button onclick="hwAnimate()" id="mode-auto"
                        class="flex-1 py-2 rounded-xl text-xs font-bold transition-all bg-cyan-500/20 text-cyan-300 border border-cyan-400/20 shadow-inner">✨
                        Tonton Animasi</button>
                    <button onclick="hwPractice()" id="mode-practice"
                        class="flex-1 py-2 rounded-xl text-xs font-bold transition-all bg-white/[.02] text-neutral-400 border border-white/[.05] hover:bg-white/[.08]">✏️
                        Latihan Nulis</button>
                </div>

                <div class="genkouyoushi rounded-2xl w-full max-w-[280px] h-[220px] mx-auto mb-4 flex items-center justify-center p-2 relative overflow-hidden bg-dark-900 shadow-inner"
                    style="touch-action: none;" id="modal-svg-container">
                ...
                <p class="text-center text-xs text-neutral-400 mb-3 min-h-[18px] font-medium" id="stroke-label">Memuat
                    papan penulisan...</p>

                <div id="practice-tools" class="hidden w-full mb-4 space-y-3">
                    <div class="flex gap-2">
                        <button onclick="resetWritingBoard()"
                            class="flex-1 py-2 rounded-xl text-xs font-bold bg-white/[.05] hover:bg-white/[.1] text-neutral-200 border border-white/[.05] transition">↺
                            Ulang</button>
                        <button onclick="scoreWritingBoard()"
                            class="flex-1 py-2 rounded-xl text-xs font-bold bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-300 border border-emerald-400/20 transition">✓
                            Nilai Tulisan</button>
                    </div>
                    <div class="bg-white/[.02] border border-white/[.05] rounded-2xl p-3">
                        ...
                        <div class="h-2 bg-dark-700 rounded-full overflow-hidden">
                            <div id="writing-score-bar"
                                class="h-full bg-gradient-to-r from-cyan-500 to-emerald-400 rounded-full transition-all duration-500"
                                style="width:0%"></div>
                        </div>
                        <p id="writing-score-desc"
                            class="mt-2 text-[10px] text-neutral-500 leading-relaxed text-center">Tulis huruf di atas
                            bayangan, lalu tekan Nilai Tulisan.</p>
                    </div>
                </div>
