...
function gemu_multi_agent_review(string $question, array $intent, array $files, array $issues, array $edits = []): array {
    ...
    $systemRating = $score;
    $watcherRating = max(0, min(100, (int)round(
        ($components['Kesesuaian file target'] / 15 * 30) +
        ($components['Keamanan perubahan'] / 25 * 25) +
        ($components['Risiko bug kecil'] / 15 * 25) +
        ($components['Test dasar lolos'] / 15 * 20)
    )));
    ...
    $frontline = [
        'role'=>'GEMU Frontline',
        'personality'=>$profiles['GEMU Frontline']['personality'] ?? '',
        'voice'=>'Ramah, lugas, dan menerjemahkan teknis menjadi keputusan yang mudah dibaca owner.',
    ...
    $backend = [
        'role'=>'GEMU Backend',
        'personality'=>$profiles['GEMU Backend']['personality'] ?? '',
        'voice'=>'Teknis, hati-hati, meminta bukti file, test, backup, dan rollback sebelum patch.',
    ...
    $system = [
        'role'=>'GEMU Sistem',
        'personality'=>$profiles['GEMU Sistem']['personality'] ?? '',
        'voice'=>'Tegas sebagai hakim skor; memutuskan boleh lanjut, analisis saja, atau blokir.',
    ...
    $watcher = [
        'role'=>'Watcher Agent',
        'personality'=>$profiles['Watcher Agent']['personality'] ?? '',
        'voice'=>'Observan dan proaktif; membawa sinyal idle, scan, feed, dan perubahan kecil ke meja diskusi.',
        'focus'=>'scan berkala, file kunci, cleanup, feed aktivitas, dan tanda anomali website',
        'rating'=>$watcherRating,
        'summary'=>'Watcher membaca konteks website sebagai radar idle: file kunci, scan terakhir, pending draft, dan saran baru.',
        'notes'=>array_values(array_filter([
            'File kandidat dipantau: '.implode(', ', array_slice($files, 0, 5)).'.',
            'Temuan terkait diskusi: '.count($issues).'.',
            'Jika idle, Watcher tetap memicu review role tanpa menulis website.',
        ])),
    ];
...
function gemu_multi_agent_discussion(string $question, array $intent, array $files, array $issues, array $edits = [], int $maxRounds = 5): array {
    ...
    for ($round = 1; $round <= $maxRounds; $round++) {
        ...
        if ($round === 1) {
            ...
            $dialogue[] = gemu_dialogue_line($label, 'Watcher Agent', 'Saya memantau konteks idle: file kandidat, scan terakhir, dan pending draft. Kalau ada sinyal berulang, saya bawa ke diskusi ini.', [
            ...
            // Ronde 2: COUNTER-ARGUMENT WAJIB — Sistem harus kritik solusi Backend
            $dialogue[] = gemu_dialogue_line($label, 'GEMU Backend → GEMU Sistem', ($evidence['hit_files'] ?? 0) > 0
                ? 'Saya menemukan bukti pada '.($evidence['hit_files'] ?? 0).' file kandidat. File yang dibaca: '.($evidence['files_read'] ?? 0).'. Saya hanya boleh patch kecil, bertahap, dan setelah test.'
                : 'Saya belum menemukan bukti target yang cukup (0 file terbukti). Saya minta Sistem menahan draft agar tidak spekulatif.', [
                'evidence'=>array_slice($evidence['items'] ?? [], 0, 5),
            ]);
            // COUNTER-ARGUMENT: Sistem WAJIB mencari kelemahan solusi Backend
            $counterArgs = [];
            if (($evidence['hit_files'] ?? 0) < 2) $counterArgs[] = 'Bukti file terlalu sedikit untuk yakin.';
            if ($patchCount > 1) $counterArgs[] = 'Lebih dari 1 patch sekaligus meningkatkan risiko.';
            if (preg_match('/auth|upload|session|password/i', $question)) $counterArgs[] = 'Perubahan menyentuh area keamanan; perlu review ekstra.';
            if (!$counterArgs) $counterArgs[] = 'Tidak ditemukan kelemahan kritis, tapi tetap wajib test sebelum draft.';
            $dialogue[] = gemu_dialogue_line($label, 'GEMU Sistem [COUNTER]', 'Kritik terhadap solusi Backend: '.implode(' ', $counterArgs).' Skor tidak boleh > 75 tanpa counter-argument terjawab.', [
                'counter_arguments'=>$counterArgs,
                'decision'=>$review['decision_text'] ?? '',
            ]);
            $dialogue[] = gemu_dialogue_line($label, 'Watcher Agent', 'Saya cek apakah saran ini muncul dari scan, feed, atau prompt owner. Bukti file: '.($evidence['files_read']??0).' dibaca, '.($evidence['hit_files']??0).' match.', []);
            $dialogue[] = gemu_dialogue_line($label, 'GEMU Frontline', $hardBlocks
                ? 'Blokir aktif. Saya jelaskan ke Darma: '.implode(' | ', array_slice($hardBlocks,0,3)).'.'
                : 'Saya ubah hasil menjadi format dashboard: skor, bukti file, counter-argument, risiko, dan status keputusan.', []);
        } else {
            // Ronde 3+: Re-evaluate dan final decision
            $review = gemu_multi_agent_review($question, $intent, $files, $issues, $edits);
            $score = (int)($review['score'] ?? 0);
            $hardBlocks = $review['hard_blocks'] ?? [];
            // Cek role disagreement: jika selisih rating > 20, diskusi ulang
            $ratings = $review['role_ratings'] ?? [];
            $ratingValues = array_values($ratings);
            $ratingSpread = count($ratingValues) >= 2 ? max($ratingValues) - min($ratingValues) : 0;
            $forceRediscuss = $ratingSpread > 20;
            if ($score < 80 || $hardBlocks || $forceRediscuss) {
                $rediscussReason = $forceRediscuss ? ' Selisih rating antar role: '.$ratingSpread.' poin (>20), wajib diskusi ulang.' : '';
                $dialogue[] = gemu_dialogue_line($label, 'GEMU Sistem', 'Skor masih '.$score.'/100.'.$rediscussReason.' Keputusan tetap analisis dulu.', [
                    'score'=>$score,
                    'hard_blocks'=>$hardBlocks,
                    'rating_spread'=>$ratingSpread,
                ]);
                $dialogue[] = gemu_dialogue_line($label, 'GEMU Backend', 'Saya tidak membuat patch spekulatif. Bukti file: '.($review['evidence']['files_read']??0).'. Langkah aman: baca file tambahan atau buat rencana teknis.', []);
                $dialogue[] = gemu_dialogue_line($label, 'Watcher Agent', 'Saya tetap aktif memantau area ini saat idle dan akan mengangkat ulang jika scan berikutnya memberi bukti baru.', []);
                $dialogue[] = gemu_dialogue_line($label, 'GEMU Frontline', 'Target belum jelas. Jawaban harus meminta fokus area dan menampilkan bukti yang kurang ke Darma.', []);
                $learning[] = 'Jika skor <80, file target belum terbukti, atau selisih rating role >20, GEMU harus berdiskusi ulang.';
            } else {
                $dialogue[] = gemu_dialogue_line($label, 'GEMU Sistem', 'Skor '.$score.'/100 dan semua gate lolos. Counter-argument terjawab. Draft boleh menunggu owner.', [
                    'score'=>$score,
                    'can_create_draft'=>true,
                ]);
                $dialogue[] = gemu_dialogue_line($label, 'GEMU Backend', 'Staged edit siap: lock, backup, hash, tulis file, test, rollback kalau gagal. File dibaca: '.($review['evidence']['files_read']??0).'.', []);
                $dialogue[] = gemu_dialogue_line($label, 'GEMU Frontline', 'Tombol ✓ Setujui dan × Tolak ditampilkan dengan laporan ringkas: skor, bukti, risiko.', []);
                $dialogue[] = gemu_dialogue_line($label, 'Watcher Agent', 'Keputusan dicatat ke feed. Memantau sampai Darma approve atau tolak.', []);
    ...
    foreach (['sistem','frontline','backend','watcher'] as $role) {
        $r = $role === 'sistem' ? $review['system'] : ($role === 'frontline' ? $review['frontline'] : ($role === 'backend' ? $review['backend'] : $review['watcher']));
...
function gemu_agent_review_text(array $review): string {
    ...
    return "Diskusi role GEMU {$rounds} ronde. Skor GEMU Sistem: {$score}/100 ({$decisionText}). Komponen: ".implode('; ', $parts).'.';
}
