...
function gemu_v19_quality_gate(string $question, array $intent, array $evidence, array $issues, array $edits = []): array {
    ...
    $filesRead = (int)($evidence['files_read'] ?? 0);
    $hitFiles  = (int)($evidence['hit_files'] ?? 0);
    ...
    // Perubahan menyentuh auth/upload/file write → wajib security review ketat
    if (preg_match('/auth|upload|password|login|session|cookie/i', $question)) $securityScore = min($securityScore, 18);

    $patchCount = count($edits);
    $riskScore = 15;
    if ($patchCount > 2) $riskScore -= 3;
    if ($patchCount > 4) $riskScore -= 5;
    if (($evidence['ambiguous'] ?? [])) $riskScore = min($riskScore, 8);
    ...
    // === HARD GATE V20 — Berbasis Bukti ===
    $hardBlocks = [];
    // Gate 1: Intent harus jelas
    if ($intentScore < 16) $hardBlocks[] = 'Intent belum cukup jelas di atas 80%.';
    // Gate 2: File target harus terbukti dibaca
    if ($filesRead === 0 && $patchCount > 0) $hardBlocks[] = 'Tidak ada file target yang dibaca. Wajib baca file sebelum draft.';
    ...
    // Gate 6: Backup gagal → jangan apply
    if (!empty($intent['needs_draft']) && $patchCount > 0 && empty($evidence['backup_ready'])) {
        $hardBlocks[] = 'Backup belum dikonfirmasi. Jangan apply tanpa backup.';
    }

    $total = $intentScore + $securityScore + $fileScore + $riskScore + $testScore + $uxScore;

    // === HARD CAP V20 — Skor tidak boleh tinggi tanpa bukti ===
    // Tidak baca file target → maks 60
    if ($filesRead === 0 && $patchCount > 0) $total = min($total, 60);
    // Tidak ada test plan → maks 70
    if ($qualityErrors > 0) $total = min($total, 59);
    if ($testScore < 8 && $patchCount > 0) $total = min($total, 70);
    // Prompt ambigu → maks 65
    if (!empty($evidence['ambiguous'])) $total = min($total, 65);
    // Tidak ada rollback/backup plan → maks 75
    if ($patchCount > 0 && empty($evidence['backup_ready'])) $total = min($total, 75);
    // Hard blocks aktif → maks 79
    if ($hardBlocks) $total = min($total, 79);
    // Confidence rendah → maks 72
    if ($confidence < 0.80 && $patchCount > 0) $total = min($total, 72);
...
function gemu_agent_slug(string $name): string {
    ...
    if (strpos($n, 'watch') !== false) return 'watcher';
    return 'sistem';
}
...
function gemu_agent_memories(): array {
    $out = [];
    foreach (['sistem','frontline','backend','watcher'] as $agent) {
...
function gemu_save_agent_session(array $review, string $question, array $intent, array $files): void {
    ...
    $session = [
        ...
        'agent_notes'=>[
            ...
            'watcher'=>$review['watcher']['summary'] ?? 'Watcher memantau scan, feed, dan perubahan idle.',
        ],
    ];
    ...
    gemu_agent_memory_add('watcher', (string)($review['watcher']['summary'] ?? 'Watcher menyimpan status diskusi dan scan terakhir.'), ['score'=>$session['last_score']]);
}
